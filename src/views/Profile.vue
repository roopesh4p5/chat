        ...
        <div class="grid-center">
            <div v-for="currentUser in user" class="max-w-sm w-72 mt-6 md:w-80 md:mt-10">
                <form @submit.prevent="handleSubmit"
                    class="w-full shadow-md rounded-md py-4 px-6  k:border border-gray-900">
                    <div class="grid-center">
                        <h1 class=" font-bold text-xl md:text-2xl">Set Up Your Profile</h1>
                    </div>
                    <div class='grid-center relative group mt-2 '>
                        ...
                        <img v-if="blob" :src="blob" alt="blob" class='h-32 w-32  border-2 rounded-full  ' />
                        <img v-if="currentUser.avatar" :src="currentUser.avatar"
                            alt="avatar" class='h-32 w-32  border-2 rounded-full ' />
                        <img v-if="!currentUser.avatar && !blob" :src="Placeholder" alt="avatar" class='h-32 w-32  border-2 rounded-full ' />
                    ...
                    <div class="pb-3 sm:pb-4">
                        <div class="flex-between">
                            ...
                            <PencilIcon class="w-4 h-4 text-black" />
                        </div>
                        <input type="text" placeholder="Username" class="form-input text-black" v-model="currentUser.username" />
                    </div>

                    <div class="pb-3 sm:pb-2">
                        <div class="flex-between">
                            <label for="email" class="form-label text-black" >Bio</label>
                            <PencilIcon class="w-4 h-4 text-black " />
                        </div>
                        <div v-if="!currentUser?.bio">
                            <input type="bio" placeholder="Make it interesting" class="form-input text-black"
